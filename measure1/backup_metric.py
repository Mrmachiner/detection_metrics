	...
	for i in range(len(lst_ground_truth)):
		if lst_ground_truth[i]['image_id'] == image_id:
			if detec == False:
				...
				_iou = iou(detection_rs, lst_ground_truth[i]['bbox'])
				if _iou > iouMax:
					iouMax = _iou
					local = i
	if iouMax > iou_thre and lst_ground_truth[local]['status'] != True:
		lst_ground_truth[local]['status'] = True
		return true_positives, lst_ground_truimage_idth
...
def match_detections(detection_results, ground_truth, overlap_threshold):
	...
	lst_FP = []		
	count_box = 0
	#detection_results[i].append(dict({True:'status'}))
	for i in range(len(detection_results)):
		# TP = np.zeros(len(detection_results[i]))
		# FP = np.zeros(len(detection_results[i]))
		# status_box_gt = np.zeros(len(detection_results[i]))
		# count_id = []
		# count_box += len(detection_results[i])
		for j in range(len(detection_results[i])):
			#print(detection_results[i][j]['image_id'])
			image_id = detection_results[i][j]['image_id']
			#count_id.append(image_id)

			#lst_box_dt_image_id = take_lst_box_imageID(detection_results[i], image_id)


			#status_box_gt = np.zeros(len(lst_box_gt_image_id))
			#print(len(lst_box_gt_image_id))
			bbox_dt = detection_results[i][j]['bbox']
			good, ground_truth[i] = single_detection_iou_multi_ground_truth(bbox_dt, ground_truth[i], image_id, overlap_threshold)
			if good == 1:
				TP[j] = 1
				#positives[index].append(good)
				t_positives.append(detection_results[i][j])
			elif good == 0:
				FP[j] = 1
				#positives[index].append(good)
				f_positives.append(detection_results[i][j])
			elif good == 3:
				f_negatives.append(detection_results[i][j])
		#status_box += status_box_gt
		print("abcd TP",TP)
		print("xyad FP",FP)
		lst_TP.append(TP)
		lst_FP.append(FP)
		# print(collections.Counter(count_id['image_id']).values())
		# count_image += len(collections.Counter(count_id).values())
		# print("count_image", count_image)
	# sum_tp = sum(TP)
	# sum_fp = sum(FP)
	# acc_FP = np.cumsum(FP)
	# acc_TP = np.cumsum(TP)
	# rec = acc_TP / count_box
...
if __name__ == "__main__":
	...
	#score_ioa = ioa(pred_box,gt_box, "aaaaa")
	#_iouAA = iou(pred_box, gt_box)
	#gnir = get_single_image_results(lst_pred_box, lst_gt_box, 0.9)
	# gnir = get_single_image_results(lst_value_pre[0], lst_value_gt[0], 0.5)
	#print(gnir)
	# lst_gnir = []
	# lst_gnir.append(gnir)
	# lst_gnir.append(gnir)
	#cpr = calc_precision_recall(lst_gnir)
	#print("avcd", cpr)
	#print(lst_value_gt[0].count())
	#print(collections.Counter(lst_value_gt[0][0]['bbox']))
	gr_id_gt, lst_key_gt = group_value_id(lst_value_gt)
	gr_id_pre, lst_key_pre = group_value_id(lst_value_pre)
	
	tp, fp = match_detections(gr_id_pre, gr_id_gt, 0.5 )
	total = tp + fp
	benchmarks(tp, fp)
